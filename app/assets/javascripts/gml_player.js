/*
 * #000000book GML player
 * Jamie Wilkinson <http://jamiedubs.com>
 * (cc) Free Art & Technology Lab
 *
 * Plays a Graffiti Markup Language tag back on a 2D canvas, in real time,
 * from the timestamps the capture app recorded.
 *
 * Replaces the old Processing.js sketch, which played one point per frame at a
 * fixed 30fps (so every tag took the same time regardless of how it was drawn),
 * sized itself to a hardcoded 600x500 (so taller tags were cut off), and
 * derived stroke weight from `sqrt(pow(dx,2), pow(dy,2))` -- sqrt takes one
 * argument, so the vertical component was discarded and width only ever
 * tracked horizontal speed. That is why old playback looked like uniform blobs.
 */
