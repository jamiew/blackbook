...
FactoryGirl.sequence :login do |i|
  "user#{i}"
end

FactoryGirl.sequence :email do |i|
  "user#{i}@000book.com"
end

FactoryGirl.sequence :device_id do |i|
  Digest::SHA1.hexdigest(i.to_s)
end

FactoryGirl.define :user do |t|
  t.login { FactoryGirl.next(:login) }
  t.name 'Test User'
  t.email  { FactoryGirl.next(:email) }
  ...
  t.iphone_uniquekey  { FactoryGirl.next(:device_id) }
end

FactoryGirl.define :admin, :parent => :user do |t|
  t.login 'admin'
  t.name 'Admin User'
  t.admin true
end
...
FactoryGirl.define :tag_from_api, :parent => :tag do |t|
...
FactoryGirl.define :tag_from_tempt1, :parent => :tag do |t|
...
FactoryGirl.define :gml_object, :class => GMLObject do |t|
  t.tag_id 1
...
# A GML application
FactoryGirl.define :visualization do |t|
  t.name "Jdubs TestTag"
  t.website "http://jamiedubs.com/yep"
  t.kind 'javascript'
end

FactoryGirl.define :favorite do |t|
  t.user {|a| a.association(:user) }
  t.object {|a| a.association(:tag) }
end

FactoryGirl.define :comment do |t|
  t.user {|a| a.association(:user) }
